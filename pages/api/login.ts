...
try{
  ...
  if(!login || !password){
    return res.status(400).json({error: 'Favor informar os dados de usuário e senha'});
  }
  ...
  res.status(500).json({error: 'Ocorreu erro ao efetuar login. Por favor, tente novamente'});
}
}
